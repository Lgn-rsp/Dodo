# LOGOS — Directory Book: /opt/logos/www
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/app.html
...
<script>document.addEventListener("DOMContentLoaded",function(){window.LOGOS_I18N&&window.LOGOS_I18N.init("#logos-lang");});</script>
</html>

```

### FILE: /opt/logos/www/.bak_20251212_093738/wallet/app.js
...
// Базовый endpoint API. Для прод‑узла это https://<домен>/api
const API = (window.API_ENDPOINT || (location.origin.replace(/\/$/, '') + '/api'));
...
function u64ToBytesBE(n) {
  if (typeof n !== 'number' || !Number.isFinite(n) || n < 0) {
    throw new Error('u64 ожидает неотрицательное число');
  }
  if (!Number.isSafeInteger(n)) {
    throw new Error('u64 вне безопасного диапазона JS');
  }
  let x = BigInt(n);
...
// /balance/:rid → { balance_micro, nonce, ... }
async function getNonce(rid) {
...
// -------------------- TX CANONICAL MESSAGE --------------------

async function buildCanonicalDigest(from, to, amount, nonce) {
  const fromBytes = enc.encode(from);
  const toBytes   = enc.encode(to);
  const bar       = enc.encode('|');
  const amountBytes = u64ToBytesBE(amount);
  const nonceBytes  = u64ToBytesBE(nonce);

  const total =
    fromBytes.length + bar.length +
    toBytes.length + bar.length +
    amountBytes.length + bar.length +
    nonceBytes.length;

  const pre = new Uint8Array(total);
  let off = 0;
  pre.set(fromBytes, off);      off += fromBytes.length;
  pre.set(bar,       off);      off += bar.length;
  pre.set(toBytes,   off);      off += toBytes.length;
  pre.set(bar,       off);      off += bar.length;
  pre.set(amountBytes, off);    off += amountBytes.length;
  pre.set(bar,         off);    off += bar.length;
  pre.set(nonceBytes,  off);

  const digest = await crypto.subtle.digest('SHA-256', pre);
  return new Uint8Array(digest);
}

// Подпись Ed25519( canonical_digest )
async function signTx(from, to, amount, nonce) {
  if (!KEYS || !KEYS.privateKey) throw new Error('ключи не загружены');
  const msg = await buildCanonicalDigest(from, to, amount, nonce);
...
function formatBalanceInfo(j) {
  const lines = [];
  if (j.rid) lines.push(`RID: ${j.rid}`);
  if (typeof j.balance_LGN === 'string') {
    lines.push(`Баланс: ${j.balance_LGN}`);
  } else if (typeof j.balance === 'number') {
    lines.push(`Баланс (сырое число): ${j.balance}`);
  }
  if (typeof j.balance_rLGN === 'string') {
    lines.push(`Баланс rLGN: ${j.balance_rLGN}`);
  }
  if (typeof j.balance_micro === 'number') {
    lines.push(`Микро‑юнитов: ${j.balance_micro}`);
  }
  if (typeof j.total_supply_LGN === 'number') {
    lines.push(`Эмиссия: ${j.total_supply_LGN} LGN`);
  }
  if (typeof j.share_of_supply_percent === 'number') {
    lines.push(`Доля от эмиссии: ${j.share_of_supply_percent.toFixed(6)}%`);
  }
  if (typeof j.nonce === 'number') {
    lines.push(`Текущий nonce: ${j.nonce}`);
  }
  return lines.join('\\n');
}
...
async function refreshBalance() {
  ...
  try {
    ...
    const j = await getJSON(`${API}/balance/${encodeURIComponent(rid)}`);
    out.textContent = formatBalanceInfo(j);
  } catch (e) {
    console.error(e);
...
if (btnSend) {
  btnSend.addEventListener('click', async () => {
    ...
    try {
      ...
      // реальный nonce: берём последний с ноды и прибавляем 1
      const currentNonce = await getNonce(RID);
      const nonce = currentNonce + 1;
      ...
      // подпись
      const sigHex = await signTx(RID, to, amount, nonce);

      // отправляем батч (одна транзакция)
      const batch = [{
        from: RID,
        to,
        amount,
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/auth.css

```
/* Используем те же переменные, что и в wallet.css */
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/auth.html
...
<head>
  ...
  <meta http-equiv="Cache-Control" content="no-store"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
  <title>LOGOS Wallet — Secure (WebCrypto + IndexedDB)</title>
...
<main>
  ...
  <section>
    ...
    <div class="muted">
      ...
      Пароль: минимум 10 символов, буквы + цифры. После создания будет показана
      резервная фраза из 16 слов — запиши и храни её.
    </div>

    <label for="createPass" style="margin-top:6px;">Новый пароль</label>
    <input id="createPass" type="password" placeholder="Минимум 10 символов, буквы+цифры"/>

    <div style="margin-top:8px;">
      <button id="btn-create">Создать новый RID + фразу</button>
    </div>

    <div id="mnemonicSection" style="display:none;">
      <h3 style="margin-top:10px;">Резервная фраза (16 слов)</h3>
      <div class="muted">
        Запиши фразу на бумагу и храни в безопасном месте. Это единственный способ
        восстановить кошелёк на новом устройстве.
      </div>

      <label for="mnemonicShow" style="margin-top:6px;">Фраза (только чтение)</label>
      <textarea id="mnemonicShow" readonly></textarea>

      <label for="mnemonicConfirm" style="margin-top:6px;">Повтори фразу для проверки ещё раз</label>
      <textarea id="mnemonicConfirm" placeholder="введите те же 16 слов через пробел"></textarea>

      <div style="margin-top:8px;">
        <button id="btn-mnemonic-ok">Я записал фразу, перейти в кошелёк</button>
      </div>
    </div>
  </section>
  ...
  <section>
    ...
    <div class="muted">
      Если устройство потеряно/очищено, используй 16-словную фразу и задай новый пароль.
      Пароль можно поменять — фраза остаётся корнем ключа.
    </div>

    <label for="restoreMnemonic" style="margin-top:6px;">Резервная фраза (16 слов)</label>
    <textarea id="restoreMnemonic" placeholder="введите 16 слов через пробел"></textarea>

    <label for="restorePass" style="margin-top:6px;">Новый пароль</label>
    <input id="restorePass" type="password" placeholder="Минимум 10 символов, буквы+цифры"/>

    <div style="margin-top:8px;">
      <button id="btn-restore">Восстановить кошелёк по фразе</button>
    </div>
  </section>
  ...
  <section>
    <h3>Сервис</h3>
    <div class="muted">
      Сброс (удаление всех локальных аккаунтов) возможен только в режиме разработчика.
    </div>
    <div style="margin-top:6px;">
      <button id="btn-reset" class="secondary">Очистить локальное хранилище</button>
    </div>
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/auth.js
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/compat.js
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/index.html
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/login.html
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/ui.js
...
### FILE: /opt/logos/www/.bak_20251212_093738/wallet/wallet.css
...
### FILE: /opt/logos/www/.bak_20251212_093738/explorer/explorer.css
...
### FILE: /opt/logos/www/.bak_20251212_093738/explorer/explorer.js
...
### FILE: /opt/logos/www/.bak_20251212_093738/explorer/index.html
...
<body>
<header>
  <h1 data-i18n="explorer.title">LOGOS LRB — исследователь</h1>
  <div class="pill">
...
<section id="global-markets"
         style="background:var(--card);border:1px solid var(--line);border-radius:14px;
                padding:12px 14px;margin:16px auto;max-width:1100px;
                font-size:13px;line-height:1.6;">
  <h3 style="margin:0 0 6px;">Рынки и языки</h3>
  <p style="margin:0 0 6px;color:var(--muted);">
    LOGOS LRB изначально спроектирован как глобальная L1. Приоритетные регионы и языки:
  </p>
  <ul style="margin:0 0 4px 18px;padding:0;">
    <li>Индия — हिंदी (Hindi)</li>
    <li>Индонезия — Bahasa Indonesia</li>
    <li>Вьетнам — Tiếng Việt</li>
    <li>Япония — 日本語 (Japanese)</li>
    <li>Китай — 中文 / 中国 (Chinese)</li>
    <li>Корея — 한국어 (Korean)</li>
    <li>Аргентина — Español (Argentina)</li>
    <li>Тайвань — 繁體中文 (Taiwan)</li>
  </ul>
</section>
